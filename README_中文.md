...
> 🧠 Jacky的智能記憶系統 — 結合 Triple-Stream Search + Knowledge Graph + Proactive Discovery
...
每個人、每個 Agent、每個使用場景都不同。Jacky是香港 QS（工料測量師），我用的系統是基於我的：

- **工作性質** — 工程項目管理、造價估算
- **技術背景** — 對 AI 有興趣但不是工程師
- **使用習慣** — 廣東說、Telegram、Obsidian
- **時間資源** — 業餘時間慢慢做

**你應該如何使用這個項目：**

1. **理解原理** — OCM Sup 為什麼要用 Triple-Stream？因为單一搜索方法不夠好
2. **按需取用** — 你可能只需要其中一兩個腳本
3. **按自身情況修改** — 你的記憶系統應該服務你的需求，不是全抄所有功能
4. **自己諗的先是你的** — 我的 evolution 方向不代表正确你，自己想出来的先最适合
...
| 问題 | 描述 |
|------|------|
| 單一搜尋方法效果差 | Vector Search 只知「相關」，不知「Jacky → works_on → 古洞站」
| 中英混合效果差 | 中文「古洞站」同英文「Kwu Tung Station」search 不到同一結果
| 假陽性多 | 向量搜索會錯誤返回Jacky，只因为他們training data 常一起出現
...
Graph: 發現「Jacky → works_on → 古洞站」關係
...
三元融合：
1. BM25 找到包含Jacky「project」的document
2. Vector 找到語義相關的「QS工作」、「造價估算」
3. Graph 發現：Jacky → works_on → 古洞站
...
Jacky问：「Star是否了解我是做QS？」

#### 😣 痛點

| 问題 | 描述 |
|------|------|
| 資訊碎片化 | 「Jacky是QS」這個事定散落在各個document入面
| 關係不明確 | 就算找到Jacky，都不知他同古洞站、OCM Sup的關係
| 需要手動維護 | 要記得哪個連接哪個，容易漏
...
沒有Graph：
→ Star說「我search到關於古洞站的document」

有Graph：
→ Star說「因为Jacky works_on 古洞站，而Star serves Jacky，
   所以我记得Jacky的project入面有古洞站」
```
...
| 问題 | 描述 |
|------|------|
| 被動等待 | Jacky不记得问，Star就不主动提
| 每次都要手動 | Jacky要記得說「幫我search古洞站」
| 不知何时觸發 | 什么關鍵詞應該觸發搜尋？
...
HIGH_PRIORITY_KEYWORDS = [
    'Jacky', '古洞站', 'Star',  # 核心 entities
    'project', '進度', '進展',  # 工作相關
    'search', '寻找', '知識',     # 動作關鍵詞
]

def should_trigger(query):
    # 如果query包含這些詞，自動觸發三元搜尋
```
...
有Hook：
Jacky：「古洞站進度是？」
Star：「古洞站是MTR東鐵線項目，目前處於BS（Building Survey）階段...」
    （自動觸發三元搜尋，提供context）
```
...
Star每日運行，但發現新的 AI news 同 Jacky的工作没有關係，不知如何使用

#### 😣 痛點

| 问題 | 描述 |
|------|------|
| 被動等问題 | 如果Jacky不问，Star就不发现新东西
| 新舊資訊不连接 | 新聞入面提到「ChatGPT」，但不知他同 OCM Sup 有什么關係
| 需要手動建立關係 | 好麻烦
...
# 規則引擎
RELATIONSHIP_RULES = {
    ('person', 'project'): 'works_on',      # Jacky + 古洞站 → works_on
...
自動推斷：Jacky → works_on → 古洞站

下次Jacky问「我最近做什么」：
Star：「你最近在做古洞站項目（BS階段）」
```
...
其他系統想用 OCM Sup 的搜尋功能，但不知點调用

#### 😣 痛點

| 问題 | 描述 |
|------|------|
| 只能在 OpenClaw 入面用 | 其他工具（Telegram bot、external script）用不到
| 不是 standard interface | 每個script有自己的調用方式
...
Star說：「我的Graph有43個entities，47個relationships」
...
|------|------|
| 看不到結構 | 全部是文字，不知哪個是中心
| 難debug | 如果關係錯了，幾乎發現不到
| 難解釋 | 向Jacky解釋「Graph是點的」好麻烦
...
Jacky问：「你何时知 OCM Sup 的？」

#### 😣 痛點

| 问題 | 描述 |
|------|------|
| 乱答 | Star當記得這麼答，但其定不记得
| 没有confidence概念 | 不知自己知多少、不知多少
| 誤導Jacky | 錯誤的自信比不记得更糟糕
...
Jacky：「你是否了解我是哪個？」
Star：「你是Jacky（confidence: 95%，from: Jacky.md）"
Jacky：「你怎么了解的？」
Star：「因为你在 Wiki 度有 entry，我记得你是QS。」
```

---

### 8. Consolidation Loop（記憶做合循環）
...
| 问題 | 描述 |
|------|------|
| Memory 過載 | 所有資訊都是「短期記憶」，没有分層 |
| 資訊衰老 | 7日前的memory可能已經過時 |
| 不主动做理 | 等 user 问先被動 search |
...
1. Episodic Buffer 入面 7 天以上的 memory distill
...
2. 7日後自動 distill:
   Jacky從 memory → Jacky.md (entity)
   「Jacky works_on 古洞站」→ relationship
3. Wiki 有結構化的知識
```
...
Star發現：舊的 memory 有時仲是search到，但可能已經不正确了

#### 😣 痛點

| 问題 | 描述 |
|------|------|
| 新舊資訊没有區分 | 2019年的memory同今日的memory權重一樣 |
| 資訊過時 | 但仍然被當成「已知事定」 |
...
没有半衰期：
Jacky：「我項目進展點？」
Star：「古洞站是QS階段」（confidence: 95%）
（可能是3個月前的狀態）

有半衰期：
Jacky：「我項目進展點？」
Star：「古洞站目前處於BS階段」（confidence: 70%）
Jacky：「之前呢？」
Star：「之前是QS階段（confidence: 35%，已衰減）
```
...
| Knowledge Graph | 關係不明確、資訊碎片化 |
| Smart Recall Hook | 被動等问題、被迫手動觸發 |
| Proactive Discovery | 發現不到新資訊同舊資訊的關係 |
| HTTP API | 其他系統用不到搜尋功能 |
| Graph Visualization | 看不到Graph結構 |
| Uncertainty Tracking | 乱答、誤導 |
| Consolidation Loop | Memory 過載、不主动做理 |
| Content Half-lives | 新舊資訊没有區分、資訊過時 |
| Active Contradiction | 矛盾没有被發現、誤導 |

**核心目標：** 讓Star了解「什么」、「為什麼」、「何时知」、「知多少」。
...
- [使用方法](#使用方法)
- [技術細節](#技術細節)
- [一步步教程](📚-一步步教程建立自己的記憶系統)
- [技術原理](TECHNICAL.md) — **深入理解每個功能背後的原理**
- [進化歷程](CHANGELOG.md) — **詳細進化史 + 決策過程 + 教訓**
- [測試報告](TEST-REPORT.md) — **所有測試結果 + 问題修復記錄**
- [Credits](#credits)
...
**OCM Sup** (OpenClaw Memory System Supreme) 是一個結合多種搜索技術的智能記憶系統，專為Jacky（香港QS專業人士）同他的 AI 助手Star設計。
...
自動識別需要觸發 Triple-Stream Search 的場景：
- Entity 名稱：Jacky、古洞站、Star、OCM Sup
...
> ⚠️ **重要提醒**：這個是 general 方向，不是你要完全跟随的 checklist。
> 每個人的情況不同，你應該按自己需要調整。
...
1. 我是哪個？（角色、背景）
2. 我服務哪個範疇？（專業領域）
3. 我慣用什么語言？（廣東說/普通說/英文）
4. 我有什么工具？（Telegram/Obsidian/Notion）
5. 我有多少時間？（業餘/全職）
```

**為什麼要知這些？**
- 你的系統應該服務你，不是你服務你的系統
- 例子：Jacky是香港 QS，用廣東說，主要用 Telegram + Obsidian
...
**根據你的 Entities 調整 KEYWORDS**：
```python
MY_ENTITIES = ['Jacky', '古洞站', 'Star']
...
按需調整：
- 如果你不需要某個 channel，拎走他
- 如果你有新的 entities type，加上去
```
...
**Q: 我應該用完所有 scripts 嗎？**
A: 不需要。揀你需要的。如果你想升級搜尋，加三元融合。如果你只想快速搜尋，BM25 就夠。
...
- [Jacky的 GitHub](https://github.com/st007097-coder)
